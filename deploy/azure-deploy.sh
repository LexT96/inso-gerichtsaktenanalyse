...
# Mount the Azure File Share at /data for SQLite persistence
echo "→ Volume Mount für SQLite-Datenbank einrichten..."
az containerapp show -n backend -g "$RESOURCE_GROUP" -o yaml > /tmp/backend-app.yaml

# Inject volume + volumeMount into the YAML if not already present
if ! grep -q "storageName: dbstorage" /tmp/backend-app.yaml 2>/dev/null; then
  python3 -c "
import yaml, sys
with open('/tmp/backend-app.yaml') as f:
    app = yaml.safe_load(f)
tmpl = app['properties']['template']
tmpl.setdefault('volumes', [])
if not any(v.get('storageName') == 'dbstorage' for v in tmpl['volumes']):
    tmpl['volumes'].append({'name': 'dbdata', 'storageName': 'dbstorage', 'storageType': 'AzureFile'})
for c in tmpl.get('containers', []):
    c.setdefault('volumeMounts', [])
    if not any(vm.get('volumeName') == 'dbdata' for vm in c['volumeMounts']):
        c['volumeMounts'].append({'volumeName': 'dbdata', 'mountPath': '/data'})
with open('/tmp/backend-app.yaml', 'w') as f:
    yaml.dump(app, f, default_flow_style=False)
" 2>/dev/null && \
  az containerapp update -n backend -g "$RESOURCE_GROUP" --yaml /tmp/backend-app.yaml -o none && \
  echo "✓ Volume Mount /data → Azure File Share eingerichtet" || \
  echo "⚠ Volume Mount konnte nicht automatisch eingerichtet werden (siehe Hinweise unten)"
  rm -f /tmp/backend-app.yaml
else
...
echo "  Backend:   https://${BACKEND_FQDN} (intern)"
echo "║                                                  ║"
echo "║  Nächste Schritte:                               ║"
echo "║  1. CORS_ORIGIN aktualisieren:                   ║"
echo "     az containerapp update -n backend \\           "
echo "       -g ${RESOURCE_GROUP} \\                      "
echo "       --set-env-vars CORS_ORIGIN=https://${FRONTEND_FQDN}"
echo "║  2. Optional: Custom Domain + TLS               ║"
echo "║                                                  ║"
echo "╚══════════════════════════════════════════════════╝"
